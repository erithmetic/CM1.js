...
// You will need to `npm install socket.io` to use this.
...
// You will need to `npm install coffee-resque` to use this. You will 
// also need to configure a resque connection:
//
//     process.env.CM1_RESQUE_URL = 'redis://example.com:4321';
CM1.useQueueAdapter = function() {
  var QueueAdapter = require('./adapters/queue-adapter');
...
CM1.prototype.adapter = function() {
  if(!CM1.adapter) CM1.adapter = useHttpAdapter();
  return CM1.adapter;
};
